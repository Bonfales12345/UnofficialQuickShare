...
public class NearbyConnectionManager : NSObject, NetServiceDelegate, InboundNearbyConnectionDelegate, OutboundNearbyConnectionDelegate{
	...
	@objc private func deviceNameDidChange(){
		updateAdvertisedDeviceName()
	}

	private func updateAdvertisedDeviceName(){
		guard let mdnsService=mdnsService else {return}
		let endpointInfo=EndpointInfo(name: DeviceNameManager.shared.currentName, deviceType: .computer)
