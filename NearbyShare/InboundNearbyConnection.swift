...
class InboundNearbyConnection: NearbyConnection{
	...
	override func processFileChunk(frame: Location_Nearby_Connections_PayloadTransferFrame) throws{
		...
		if frame.payloadChunk.body.count>0{
			...
			finishReceivingFile(id: id, file: fileInfo)
	...
	private func processIntroductionFrame(_ frame:Sharing_Nearby_Frame) throws{
		...
		if frame.v1.introduction.fileMetadata.count>0 && frame.v1.introduction.textMetadata.isEmpty{
			let downloadsDirectory=SaveLocationManager.shared.currentSaveLocationURL()
	...
	private func shouldAutoOpen(_ file:InternalFileInfo) -> Bool{
		let mimeType=file.meta.mimeType
		if mimeType.starts(with: "image/") || mimeType=="application/pdf" || mimeType=="text/plain"{
			return true
		}

		switch file.destinationURL.pathExtension.lowercased(){
		case "png","jpg","jpeg","gif","heic","webp","bmp","tiff","pdf","txt":
			return true
		default:
			return false
		}
	...
	private func finishReceivingFile(id:Int64, file:InternalFileInfo){
		if shouldAutoOpen(file){
			DispatchQueue.main.async {
				NSWorkspace.shared.open(file.destinationURL)
			}
		}
	}
